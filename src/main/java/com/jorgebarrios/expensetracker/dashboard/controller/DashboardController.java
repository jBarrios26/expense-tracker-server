...
import com.jorgebarrios.expensetracker.dashboard.model.response.SpentByDayResponse;
...
public class DashboardController {
    ...
    public ResponseEntity<LastTransationResponse> getLastTransactions(
        ...
        return ResponseEntity.ok(new LastTransationResponse(budgetExpenseList.stream()
    ...
    @GetMapping("/categories/spent-by-day/{userId}")
    public ResponseEntity<SpentByDayResponse> getSpentByDay(
            @PathVariable String userId
    ) {
        List<DayExpenseDTO> weekDayExpenses =
                dashboardService.getTotalSpentByWeekDay(UUID.fromString(userId));
        return ResponseEntity.ok(new SpentByDayResponse(weekDayExpenses));
    }
}
